<?php
    ...
    function getAllUsers($pdo) {
        $sql = "SELECT * FROM user_credentials ORDER BY username";
        $stmt = $pdo->prepare($sql);
    ...
    function deleteDocument($pdo, $docId) {
        ...
        $query1 = $stmt->execute([$docId]);

        $query2 = deleteDocumentActivityLogs($pdo, $docId);
        $query3 = deleteDocumentShares($pdo, $docId);
        $query4 = deleteDocumentMessages($pdo, $docId);
    ...
    // delete document activity logs
    function deleteDocumentActivityLogs($pdo, $docId) {
    ...
    function deleteDocumentShares($pdo, $docId) {
    ...
    function deleteDocumentMessages($pdo, $docId) {
    ...
    // change title
    function changeDocumentTitle($pdo, $docId, $newTitle) {
        $sql = "UPDATE documents SET title = ? WHERE document_id = ?";
        $stmt = $pdo->prepare($sql);

        $query = $stmt->execute([$newTitle, $docId]);
