<?php
    ...
    function getAllUsers($pdo) {
        $sql = "SELECT * FROM user_credentials WHERE role = 'User' ORDER BY user_id DESC";
        $stmt = $pdo->prepare($sql);
    ...
    function getDocumentById($pdo, $docId, $owner_id) {
        $sql = "SELECT * FROM documents WHERE document_id = ? AND owner_id = ?";
        $stmt = $pdo->prepare($sql);

        $query = $stmt->execute([$docId, $owner_id]);
    ...
    // save document
    function saveDocument($pdo, $content, $docId) {
